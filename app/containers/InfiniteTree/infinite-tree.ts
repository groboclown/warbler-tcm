...
import classNamesX from 'classnames';
...
const error = (format: string, ...args: any[]) => {
    let argIndex = 0;
...
const ensureNodeInstance = (node: Node | null) => {
...
const createRootNode = (rootNode?: Node | null): Node => {
    return extend(rootNode || ({} as Node), {
        parent: null,
        children: [],
        state: {
            depth: -1,
...
export interface RowHeightFunction<T> {
  (node: Node, tree: InfiniteTree<T>, index: number): number
}

export interface RowRendererFunction<T> {
  (node: Node, tree: InfiniteTree<T>, index: number): JSX.Element
}
...
export interface LoadNodesFunction<T> {
  (parentNode: Node, done: LoadDoneFunction<T>): void
}
...
export interface TreeOptions<T> {
  ...
  droppable?: {accept?: Function, drop?: Function, hoverClass?: string}
...
export class InfiniteTree<T> extends events.EventEmitter {
    options: TreeOptions<T> = {
        autoOpen: false,
        droppable: undefined,
        shouldLoadNodes: undefined,
        loadNodes: undefined,
        rowRenderer: defaultRowRenderer,
        selectable: true,
        shouldSelectNode: undefined,
    ...
    clusterize: Clusterize | null = null;
    ...
    draggableTarget: HTMLElement | null = null;
    droppableTarget: HTMLElement | null = null;
    ...
    contentListener = {
        'click': (event: Event) => {
            ...
            setTimeout(() => {
                ...
                while (itemTarget && itemTarget.parentElement !== this.contentElement) {
                    ...
                    itemTarget = (itemTarget as any).parentElement;
                }
        ...
        'dragstart': (event: Event) => {
            ...
            this.draggableTarget = (event.target as HTMLElement) || event.srcElement;
        },
        ...
        'dragenter': (event: Event) => {
            ...
            const { accept = undefined, hoverClass = '' } = this.options.droppable || {};
        ...
        'drop': (event: Event) => {
            ...
            const { accept = undefined, drop = undefined, hoverClass = '' } = this.options.droppable || {};
    ...
    constructor(el: HTMLElement | TreeOptions<T>, options?: TreeOptions<T>) {
        super();

        if (el instanceof HTMLElement) {
            (options as any) = { ...options, el };
    ...
    create() {
        if (this.options.el) {
            ...
            if (this.options.layout === 'table') {
                ...
                tableElement.className = classNamesX(
                    'infinite-tree',
                    'infinite-tree-table'
                );
            ...
            this.scrollElement.className = classNamesX(
                'infinite-tree',
                'infinite-tree-scroll'
            );
            this.contentElement.className = classNamesX(
                'infinite-tree',
            ...
            if (this.options.droppable) {
                addEventListener(document as any, 'dragstart', this.contentListener.dragstart);
                addEventListener(document as any, 'dragend', this.contentListener.dragend);
    ...
    destroy() {
        ...
        if (this.options.el && this.contentElement) {
            ...
            if (this.options.droppable) {
                removeEventListener(document as any, 'dragstart', this.contentListener.dragstart);
                removeEventListener(document as any, 'dragend', this.contentListener.dragend);
    ...
    addChildNodes(newNodeList: T[], index?: number | Node, parentNode?: Node): boolean {
        let newNodes: (T & Node)[] = ([] as (T & Node)[]).concat((newNodeList || []) as (T & Node)[]); // Ensure array
        ...
        parentNode.children.splice.apply(parentNode.children, ([index, 0] as any[]).concat(newNodes));
        ...
        for (let i = 0; i < nodes.length; ++i) {
            const node = nodes[i];
            rows[i] = this.options.rowRenderer(node, this, i);
        }
        ...
        if (parentNode === this.state.rootNode) {
            ...
            if (parentOffset >= 0) {
                if (parentNode.state.open === true) {
                    // Update nodes & rows
                    this.nodes.splice.apply(this.nodes, ([parentOffset + 1, deleteCount] as any[]).concat(nodes));
                    this.rows.splice.apply(this.rows, ([parentOffset + 1, deleteCount] as any[]).concat(rows));
                }

                // Update the row corresponding to the parent node
                this.rows[parentOffset] = this.options.rowRenderer(parentNode, this, parentOffset);
            }
        }
    ...
    // @param {Node} parentNode The Node object that defines the parent node.
    // @return {boolean} Returns true on success, false otherwise.
    appendChildNode(newNode: T, parentNode: Node): boolean {
        ...
        const newNodes = ([] as (T & Node)[]).concat(((newNode as T & Node) || [])); // Ensure array
    ...
    checkNode(node: Node, checked?: boolean): boolean {
        ...
        const updateChildNodes = (parentNode: Node) => {
            ...
            while (childNode) {
                ...
                if (childNode.hasChildren()) {
                    ...
                    while ((childNode.getNextSibling() === null) && (childNode.parent !== parentNode)) {
                        // unnecessary extra null check to make TS happy
        ...
        const updateParentNodes = (childNode: Node) => {
            ...
            while (parentNode && parentNode.state.depth >= 0) {
                ...
                for (let i = 0; i < len; ++i) {
                    // Null check is done with the len value, so force the
                    // type.
                    const childNode = (parentNode.children as Node[])[i];
    ...
    // @return {boolean} Returns true on success, false otherwise.
    closeNode(node: Node, options?: EventOptions): boolean {
        const {
            async = false,
            asyncCallback = noop,
            silent = false
        ...
        this.rows[nodeIndex] = this.options.rowRenderer(node, this, nodeIndex);
        // Update list
        this.update();
        ...
        const fn = () => {
            ...
            if (this.state.selectedNode && (this.state.selectedNode !== node)) {
                ...
                if ((rangeFrom <= selectedIndex) && (selectedIndex <= rangeTo)) {
                    this.selectNode(node as Node & T, options);
                }
            }
            ...
            const total = node.state.total;
            for (let p: Node | null = node; p !== null; p = p.parent) {
    ...
    filter(predicate: string | NodeFilter<T>, options?: FilterOptions): void {
        ...
        const traverse = (node: Node, filterNode: boolean = false) => {
        ...
        for (let i = 0; i < this.nodes.length; ++i) {
            ...
            this.rows[i] = this.options.rowRenderer(node, this, i);
        }
    ...
    flattenChildNodes(parentNode: Node): (Node & T)[] {
        // Defaults to rootNode if the parentNode is not specified
        parentNode = parentNode || this.state.rootNode;
    ...
    // @return {array} Returns an array of Node objects containing all the child nodes of the parent node.
    getChildNodes(parentNode?: Node | null): (Node & T)[] {
        // Defaults to rootNode if the parentNode is not specified
        parentNode = parentNode || this.state.rootNode;
    ...
    getNodeById(id: number | string | undefined | null): (Node & T) | null {
    ...
    getNodeFromPoint(x: number, y: number): (Node & T) | null {
    ...
    // @return {boolean} Returns true on success, false otherwise.
    insertNodeAfter(newNode: any, referenceNode: Node): boolean {
    ...
    // @param {Node} referenceNode The Node object that defines the reference node.
    // @return {boolean} Returns true on success, false otherwise.
    insertNodeBefore(newNode: any, referenceNode: Node): boolean {
        if (!ensureNodeInstance(referenceNode)) {
            return false;
        }
    ...
    // @param {number} [index] The 0-based index of where to insert the child node.
    // @return {boolean} Returns true on success, false otherwise.
    moveNodeTo(node: Node, parentNode: Node, index?: number): boolean {
    ...
    // @return {boolean} Returns true on success, false otherwise.
    openNode(node: Node, options?: EventOptions): boolean {
        const {
            async = false,
            asyncCallback = noop,
            silent = false
            ...
            setTimeout(() => {
                this.options.loadNodes(node, (err, nodes, done = noop) => {
                    ...
                    if (err || nodes.length === 0) {
                        ...
                        // Update the row corresponding to the node
                        this.rows[currentNodeIndex] = this.options.rowRenderer(node, this, currentNodeIndex);
                        // Update list
                        this.update();
                    ...
                    if (node.hasChildren()) {
                        ...
                        this.openNode(node, {
                            ...
                            asyncCallback: () => {
                                ...
                                // Update the row corresponding to the node
                                this.rows[openedNodeIndex] = this.options.rowRenderer(node, this, openedNodeIndex);
                                // Update list
                                this.update();
    ...
    // @return {boolean} Returns true on success, false otherwise.
    removeChildNodes(parentNode: Node): boolean {
        if (!ensureNodeInstance(parentNode)) {
            return false;
        }
        ...
        if (parentNodeIndex >= 0) {
            ...
            // Update the row corresponding to the parent node
            this.rows[parentNodeIndex] = this.options.rowRenderer(parentNode, this, parentNodeIndex);
        }
    ...
    // @param {boolean} [options.silent] Pass true to prevent "selectNode" event from being triggered.
    // @return {boolean} Returns true on success, false otherwise.
    removeNode(node: Node, options?: EventOptions): boolean {
        if (!ensureNodeInstance(node)) {
            return false;
        }
        ...
        { // Update open nodes and lookup table
            this.state.openNodes = this.state.openNodes.filter((node) => {
                return (removedNodes.indexOf(node as Node & T) < 0) && node.state.open;
            });
    ...
    scrollToNode(node: Node): boolean {
        ...
        while (firstChild) {
            const fa: any = firstChild as any
            const className = fa.className || '';
            if (className.indexOf('clusterize-extra-row') < 0 && (fa.offsetHeight > 0)) {
                break;
            }
            firstChild = firstChild.nextSibling;
        }
        // If all items in the list is the same height, it can be calculated by nodeIndex * height.
        const offsetHeight = (firstChild && (firstChild as any).offsetHeight) || 0;
        if (offsetHeight > 0) {
        ...
        const nodeEl: HTMLElement | null = this.contentElement.querySelector(nodeSelector);
        if (nodeEl) {
    ...
    // @return {boolean} Returns true on success, false otherwise.
    selectNode(node: Node & T | null, options?: SelectOptions): boolean {
        ...
        if (node === null) {
            ...
            if (this.state.selectedNode) {
                ...
                this.rows[selectedIndex] = this.options.rowRenderer(selectedNode, this, selectedIndex);
                this.state.selectedNode = null;
    ...
    // @return {boolean} Returns true on success, false otherwise.
    swapNodes(node1: Node, node2: Node): boolean {
        ...
        // null safety
        parentNode1.children = parentNode1.children || []
        const parentNode2 = node2.parent;
        // null safety
        parentNode2.children = parentNode2.children || []

        for (let p: Node | null = parentNode1; p !== null; p = p.parent) {
            if (p === node2) {
                error('Cannot swap two nodes with one being an ancestor of the other.');
                return false;
            }
        }
        for (let p: Node | null = parentNode2; p !== null; p = p.parent) {
            if (p === node1) {
                error('Cannot swap two nodes with one being an ancestor of the other.');
                return false;
            }
    ...
    // @return {boolean} Returns true on success, false otherwise.
    toggleNode(node: Node, options?: EventOptions): boolean {
        if (!ensureNodeInstance(node)) {
            return false;
        }
    ...
    toString(node?: Node | null): string {
                ...
                for (let i = 0; i < node.children.length; ++i) {
                    ...
                    Object.keys(node).forEach((key) => {
                        ...
                        if (key === 'children') { // traverse child nodes
                            list.push('"' + key + '":' + traverse(node.children && node.children[i] || null));
                            return;
                        }
    ...
    updateNode(node: Node, data: any, options?: UpdateOptions): void {
        if (!ensureNodeInstance(node)) {
            return;
        }
        ...
        if (nodeIndex >= 0) {
            ...
            if (!shallowRendering) {
                ...
                for (let index = rangeFrom; index <= rangeTo; ++index) {
                    this.rows[index] = this.options.rowRenderer(this.nodes[index], this, index);
                }
            }
